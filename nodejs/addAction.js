...
// IOSTオブジェクトに実行アカウントオブジェクトをセットする。
iost.setAccount(account);

// トランザクションを作成する。
const tx = new IOST.Tx(1, 2000000);
// トランザクションにアクションを追加する。
tx.addAction('token.iost', 'balanceOf', JSON.stringify(['iost', 'alice']));
// トランザクションにアクションを追加する。
tx.addAction('token.iost', 'balanceOf', JSON.stringify(['iost', 'bobby']));
// チェーンIDをセットする。
tx.setChainID(1020);
